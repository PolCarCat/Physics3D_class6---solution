...
j1Render::j1Render(Application* app, bool start_enabled) : Module(app, start_enabled)
{
	background.r = 0;
	background.g = 0;
	background.b = 0;
...
bool j1Render::Init()
{
	...
	{
		camera.w = (float)App->window->screen_surface->w;
		camera.h = (float)App->window->screen_surface->h;
		camera.x = 0.0f;
		camera.y = 0.0f;
	}
...
bool j1Render::Blit(SDL_Texture* texture, int x, int y, const SDL_Rect* section, float speed, double angle, int pivot_x, int pivot_y, bool scaling, bool h_flip,bool use_camera, SDL_Color color) const
{
	...
	float scale = App->window->GetScale();
	SDL_Rect rect;
	if (scaling)
	{
		if (use_camera)
